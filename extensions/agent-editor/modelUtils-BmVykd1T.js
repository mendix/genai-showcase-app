import { c as r } from "./DesignSystemTokens-BZrmEyPo.js";
const d = "data:image/png;base64,", E = "data:image/png;base64,", m = "data:image/png;base64,", C = "data:image/png;base64,", u = async (A, e) => {
  const a = await s(A, e);
  if (!a)
    return await A.ui.messageBoxes.show("error", "Could not find constant.", "This constant might have been deleted or renamed. Please update the tool constant selection.");
  await A.ui.editors.editDocument(a.$ID);
}, i = (A) => {
  if (A)
    return A.defaultValue;
}, g = async (A, e) => (await A.runtime.configuration.getConstants()).find((t) => t.constantName === e), I = async (A, e) => {
  if (!e)
    return;
  const a = await g(A, e.qualifiedName), n = await s(A, e), t = i(n);
  if (!a)
    return t;
  if (!a.isPrivate)
    return a.value || void 0;
}, B = (A) => {
  var e;
  return ((e = A == null ? void 0 : A.type) == null ? void 0 : e.$Type) === "DataTypes$StringType";
}, s = async (A, e) => {
  if (e)
    try {
      const a = await A.app.model.constants.loadAll((n) => n.$ID === e.documentId);
      return a[0] && a[0].name === e.qualifiedName.split(".").slice(-1)[0] ? a[0] : void 0;
    } catch {
      return;
    }
}, w = (A, e) => async (a) => {
  const n = r(A);
  try {
    await A.app.model.customBlobDocuments.updateDocumentContent(e, a);
  } catch (t) {
    n.error("Failed to save model document:", t), await A.ui.notifications.show({
      title: "Failed to save model document.",
      message: t.message
    });
  }
};
function l() {
  return {
    type: "",
    name: "",
    displayName: "",
    provider: "MxCloudGenAI",
    // Preselect MxCloudGenAI as default provider
    providerFields: o()
  };
}
const o = () => ({
  environment: "",
  deepLinkURL: "",
  keyId: "",
  keyName: "",
  resourceName: ""
}), p = (A) => {
  const e = A.providerFields;
  return {
    ...l(),
    type: A.type,
    provider: A.provider,
    providerFields: {
      ...o(),
      key: e.key
    }
  };
};
export {
  p as a,
  E as b,
  o as c,
  l as d,
  m as e,
  C as f,
  I as g,
  u as h,
  s as i,
  B as j,
  g as k,
  i as l,
  d as m,
  w as u
};
//# sourceMappingURL=modelUtils-BmVykd1T.js.map
