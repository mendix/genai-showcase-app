...
const k = ({
  ...
  p.current = o, c.useEffect(() => {
    const f = ({ documents: x }) => {
      const d = [];
      for (const a of x)
        m.current && a.documentType === w && a.moduleName && m.current.includes(a.moduleName) ? d.push("domainModel") : p.current && a.documentType === C ? d.push("projectSettings") : u.current.includes(a.documentId) && d.push(a.documentId);
