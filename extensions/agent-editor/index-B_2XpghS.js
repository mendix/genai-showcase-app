...
import { m as b } from "./modelIcons-DMizWKtH.js";
...
import { i as T, m as A, h as y, k as g, j as _e, d as Se, e as Be, g as ve } from "./documentInitUtils-CLdpbIQ7.js";
...
const qt = 300, f = {
  ...
  le,
  ue
], me = "AE301", pe = "AE302", Ee = "AE303", fe = "AE304", $e = [
  me,
  pe,
  Ee,
  fe
...
async function rt(o, e) {
  ...
  const i = await Ae(o, n), s = e.variables.filter((a) => !i.includes(a.key)).map((a) => a.key);
...
async function ht(o, e) {
  const t = await o.app.model.customBlobDocuments.getDocumentsOfType(A);
  for (const r of t) {
    const n = await o.app.model.customBlobDocuments.getDocumentById(r.id);
      ...
      if (await t.app.model.customBlobDocuments.registerDocumentType({
        ...
        i.runtime.controller.addEventListener("connectionChanged", (c) => {
          ...
          async ({ messageId: c, message: Te }) => {
            Te.type === f.GetConnectionState && await i.ui.messagePassing.sendResponse(c, {
              isConnected: N
            });
          }
          ...
          try {
            await i.runtime.controller.executePreviewAction(
              He.PreviewAgentSync,
              {}
            ), await i.ui.messagePassing.sendMessage({ type: f.SyncSuccessful });
          } catch {
            await i.ui.messagePassing.sendMessage({ type: f.SyncFailed });
          }
