...
async function W(n, t, e) {
  const A = e.systemPrompt.match(/\{\{([^}]+)\}\}/g) || [], o = e.userPrompt ? e.userPrompt.match(/\{\{([^}]+)\}\}/g) || [] : [], a = t === z, r = [...A, ...o];
  if (r.length === 0)
    return Promise.resolve([]);
  const w = r.map((s) => s.replace(/\{\{|\}\}/g, "")), F = [...new Set(w)], f = e.variables, M = F.map((s) => f.find((T) => T.key === s) || {
    key: s,
...
async function V(n, t, e) {
  const A = await Q(n, e), o = await B(e, n);
  return t.map((a) => {
    const r = o.includes(a.key);
    return {
      ...a,
      isAttributeInEntity: A === !1 || r
    };
  });
}
const Q = async (n, t) => t.entity ? await i(n, t.entity) !== void 0 : !0, q = async (n, t) => t.variables.length === 0 ? [] : !t.entity || !await i(n, t.entity) ? [] : t.variables.filter((e) => !e.isAttributeInEntity).map((e) => e.key), B = async (n, t) => {
  if (!n.entity)
    return [];
  const e = await i(t, n.entity);
  return e ? (await b(t, e)).sort() : [];
}, j = async (n, t) => (await B(t, n)).filter((A) => !t.variables.find((o) => o.key === A)), Z = {
  Microflow: "Microflow",
  MCP: "MCP"
}, S = ["v2025_03_26", "v2024_11_05"], v = S[0];
function x() {
  return {
    endpoint: "",
    protocolVersion: v
...
}, $ = "extension/agent-editor", g = "agenteditor.agent", E = "agenteditor.model", C = "agenteditor.consumedMCPService", y = "agenteditor.knowledgebase";
class K {
  async loaded(t) {
    const e = h(t), A = p(e);
    try {
...
//# sourceMappingURL=index-DfLbwebo.js.map
