...
var Cr = (o, e, t) => e in o ? xr(o, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : o[e] = t;
var K = (o, e, t) => Cr(o, typeof e != "symbol" ? e + "" : e, t);
import { getComponentFramework as hr } from "@mendix/component-framework";
...
const jr = 5;
function Fr(o, e = {}) {
  const t = e.maxDepth ?? jr, n = [], c = /* @__PURE__ */ new Set();
  let u = o, s = 0;
  for (; u != null && s < t; ) {
    const i = u == null ? void 0 : u.cause;
    if (i == null || c.has(i))
      break;
    c.add(i), n.push(pr(i)), u = i, s += 1;
  }
  return n;
}
function pr(o) {
...
function P(o, e) {
  ...
  return e != null && Sr(t, e), U(t.showHidden) && (t.showHidden = !1), U(t.depth) && (t.depth = 2), U(t.colors) && (t.colors = !0), U(t.customInspect) && (t.customInspect = !0), t.colors && (t.stylize = $r), Z(t, o, t.depth);
}
P.colors = lr;
P.styles = {
  special: "cyan",
...
function Vr(o, e, t, n, c) {
  const u = [];
  for (let s = 0, i = e.length; s < i; ++s)
    Lr(e, String(s)) ? u.push(sr(o, e, t, n, String(s), !0)) : u.push("");
  return c.forEach((s) => {
...
`, `
  ...
  o.seen.push(e);
  let g;
  return i ? g = Vr(o, e, t, u, c) : g = c.map((p) => sr(o, e, t, u, p, i)), o.seen.pop(), Yr(g, s, d);
}
function sr(o, e, t, n, c, u) {
  ...
  if (d.get ? d.set ? i = o.stylize("[Getter/Setter]", "special") : i = o.stylize("[Getter]", "special") : d.set && (i = o.stylize("[Setter]", "special")), Lr(n, c) || (s = "[" + c + "]"), i || (o.seen.indexOf(d.value) < 0 ? (Gr(t) ? i = Z(o, d.value, void 0) : i = Z(o, d.value, t - 1), i.indexOf(`
`) > -1 && (u ? i = i.split(`
`).map((g) => "  " + g).join(`
`).substr(2) : i = `
` + i.split(`
`).map((g) => "   " + g).join(`
`))) : i = o.stylize("[Circular]", "special")), U(s)) {
    if (u && c.match(/^\d+$/))
      return i;
...
function Qr() {
  ...
  const u = {
    ...
    getCallerStackFrame(a, l = new Error()) {
      ...
      if (B.length === 0)
        return {};
      const f = Ir(B, n), k = Number.isFinite(a) && a >= 0, y = gr(k ? a : f, B.length);
      return B[y] ?? {};
    },
    getErrorTrace(a) {
      return or(a, (l) => s(l));
...
`), L = A(a), D = y ? a : L;
  ...
  return Ar() === "test" && (u.__resetWorkingDirectoryCacheForTests = () => {
    c = void 0;
  }), u;
  function s(a) {
    return t ? d(a) : i(a);
  ...
  function i(a) {
    ...
    const H = V(M), Q = H.length > 0 ? H : M, dr = Q.split(/\\|\//), er = dr[dr.length - 1], wr = er && D ? `${er}:${D}` : void 0, Tr = Q && D ? `${Q}:${D}` : void 0;
    return {
      fullFilePath: G,
      fileName: er,
      fileNameWithLine: wr,
  ...
  function h(a) {
    return a && (o.name === "browser" || o.name === "worker") && Jr();
  }
  function A(a) {
    return a.replace(Xr, "");
  }
  function I(a, l) {
    ...
    if (l == null)
      return { text: "", styles: [] };
    const { template: B, placeholders: f } = kr(a, l), k = [], y = [];
    let G = 0;
    const L = /{{(.+?)}}/g;
    let D;
    for (; (D = L.exec(B)) != null; ) {
...
}, re = /(?:(?:file|https?|global code|[^@]+)@)?(?:file:)?((?:\/[^:/]+){2,})(?::(\d+))?(?::(\d+))?/, C = Qr();
class ee {
...
{{errorStack}}`,
  ...
  _prettyFormatLogObjMeta(e) {
    return kr(this.settings, e).text;
  }
}
class oe extends ee {
  constructor(e, t) {
...
function fe(o) {
  const e = te(), t = new ne(e, o), n = new oe({
    minLevel: 0,
...
var ae = Object.defineProperty, le = (o, e) => ae(o, "name", { value: e, configurable: !0 });
...
let ce;
const he = { name: "light", get devTokens() {
...
let se;
const ue = { name: "dark", get devTokens() {
  return se ?? (se = new q(this));
}, get colorsAccentBlack() {
  return r.colorsDarkBrandGrey0;
}, get colorsAccentDefaultBlue() {
  return r.colorsDarkBrandBlue7;
