import { r as c, L as s, I as t, ad as j, ae as y, S as v } from "./Icon-Y0B9OBpL.js";
...
const $ = ({
  ...
  p.current = o, c.useEffect(() => {
    const f = ({ documents: x }) => {
      const l = [];
      for (const a of x)
        m.current && a.documentType === E && a.moduleName && m.current.includes(a.moduleName) ? l.push("domainModel") : p.current && a.documentType === w ? l.push("projectSettings") : u.current.includes(a.documentId) && l.push(a.documentId);
      l.length > 0 && d.current(l);
    };
...
function k(e) {
  return {
    async loaded(n, i) {
      const o = C(n), { documentId: r } = i, d = document.getElementById("root");
      if (!d) throw new Error("Failed to find the root element");
      y.createRoot(d).render(
