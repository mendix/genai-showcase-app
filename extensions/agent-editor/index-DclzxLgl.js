import { a as ge, d as O } from "./jsx-runtime-CWOFuNcx.js";
import { getComponentFramework as ye } from "@mendix/component-framework";
import { k as h } from "./knowledgebaseIcons-B7aB_ZnQ.js";
import { m as w } from "./modelIcons-tHNcUPkT.js";
import { m as N } from "./toolIcons-B55yYne7.js";
import { T as De, e as Te, f as Ce, i as Re, j as Oe, g as he, C as we } from "./toolUtils-3GFu_U6Y.js";
import { g as b } from "./customBlobDocumentUtils-DRHCWWx0.js";
import { C as Ne, a as Ie } from "./compatibility-B4OgVO3B.js";
import { a as D, i as T } from "./constantUtils-D0RXxXWn.js";
import { h as y, m as p, g as A, k as f, i as be, b as Me, d as ke, f as Se } from "./documentInitUtils-Bwe4f_Cu.js";
import { M as Be, E as xe, b as _e, C } from "./studioPro-D79eumit.js";
function ve(n) {
  const e = ye(n);
  return {
    network: {
      httpListener: e.getApi("mendix.HttpListenerApi")
    },
    analytics: e.getApi("mendix.AnalyticsApi")
  };
}
const M = "agenteditor/settings.json", Le = "onboardingView", Pe = "Get Started", u = {
  AgentEditor: { id: "agent-editor", caption: "Agent Editor" },
  Onboarding: { id: "agent-editor.onboarding", caption: "Onboarding" },
  Compatibility: { id: "agent-editor.compatibility", caption: "Compatibility" }
}, _t = {
  CallingTheAgent: "https://docs.mendix.com/agents/how-to/create-agent-with-agent-editor/#calling-the-agent",
  CreateFirstAgent: "https://docs.mendix.com/agents/how-to/create-agent-with-agent-editor/",
  Feedback: "https://forms.office.com/Pages/ResponsePage.aspx?id=zTuuOHmV1E-t2rQuFJXVWr6T473xSWFBjtAtbbq1JPpUNE5NTjRRUkZFWVNKMU1OTU9BTFdJS01NTi4u",
  GenAIConcepts: "https://docs.mendix.com/agents/get-started/",
  MxAcademy: "https://docs.mendix.com/agents/#getting-started",
  MxCloudGenAIPortal: "https://docs.mendix.com/agents/mx-cloud-genai/Navigate-MxGenAI/",
  MxCloudGenAIResourcePacks: "https://docs.mendix.com/agents/mx-cloud-genai/resource-packs/",
  Newsletter: "https://forms.office.com/pages/responsepage.aspx?id=jcfjtDuO2Ea8VlVA2iO6Tch-n5LSBBxDiaKK50UnnwhUQTQ1VEFMTlZBQ0VYVlhZR0ZZWUUzT1NCNC4u&route=shorturl"
};
function Ge() {
  return {
    shown: !1,
    version: ""
  };
}
async function Ue(n) {
  try {
    const t = (await n.app.model.modules.getModules()).find((r) => r.name === "AgentEditor");
    if (t?.appStoreVersion)
      return t.appStoreVersion;
  } catch {
  }
  return "2.0.0";
}
function qe() {
  return {};
}
async function k(n) {
  let e = "";
  try {
    e = await n.app.files.getFile(M) || "";
  } catch {
  }
  const t = e.trim();
  if (t !== "")
    try {
      return JSON.parse(t);
    } catch {
    }
  const r = qe();
  return await S(n, r), r;
}
async function S(n, e) {
  await n.app.files.putFile(M, JSON.stringify(e, null, 2));
}
function Fe(n, e) {
  return n.onboarding?.shown !== !0 || n.onboarding?.version !== e;
}
async function ze(n, e) {
  const t = await k(n), r = {
    ...t,
    onboarding: {
      ...Ge(),
      ...t.onboarding,
      ...e
    }
  };
  await S(n, r);
}
async function Ke(n, e) {
  await ze(n, { shown: !0, version: e });
}
const vt = 300, E = {
  ConnectionChanged: "ae_connectionChanged",
  GetConnectionState: "ae_getConnectionState",
  SyncSuccessful: "ae_syncSuccessful",
  SyncFailed: "ae_syncFailed"
}, Ve = {
  PreviewAgentSync: "preview_agent_sync",
  PreviewAgentTest: "preview_agent_test"
}, g = "data:image/png;base64,", Lt = "data:image/png;base64,", Ye = {
  Chat: "Chat",
  Task: "Task"
}, Pt = {
  Agent: "agent",
  User: "user",
  Error: "error"
}, Gt = {
  Disabled: "Disabled",
  Enabled: "Enabled",
  SyncNeeded: "SyncNeeded",
  SyncFailed: "SyncFailed",
  SyncSuccessful: "SyncSuccessful"
}, Ut = {
  Build: "Build",
  Playground: "Playground"
}, B = "AE001", x = "AE002", _ = "AE003", v = "AE004", L = "AE005", P = "AE006", G = "AE007", U = "AE008", q = "AE009", F = "AE010", z = "AE011", K = "AE012", V = "AE013", Y = "AE014", X = "AE015", H = "AE016", Q = "AE017", j = "AE018", Xe = [
  B,
  x,
  _,
  v,
  L,
  P,
  G,
  U,
  q,
  F,
  z,
  K,
  V,
  Y,
  X,
  H,
  Q,
  j
], J = "AE101", W = "AE102", Z = "AE103", $ = "AE104", He = [
  J,
  W,
  Z,
  $
], ee = "AE201", te = "AE202", ne = "AE203", oe = "AE204", re = "AE205", se = "AE206", ie = "AE207", ae = "AE208", Qe = [
  ee,
  te,
  ne,
  oe,
  re,
  se,
  ie,
  ae
], ce = "AE301", de = "AE302", le = "AE303", ue = "AE304", je = [
  ce,
  de,
  le,
  ue
], me = async (n, e) => {
  if (e.generalization.$Type === "DomainModels$NoGeneralization")
    return e.attributes.map((o) => o.name);
  const t = await pe(n, e);
  if (!t)
    return e.attributes.map((o) => o.name);
  const r = await me(n, t);
  return e.attributes.map((o) => o.name).concat(r);
}, Je = async (n, e) => {
  if (n.generalization.$Type === "DomainModels$NoGeneralization")
    return n.generalization.persistable;
  const t = await pe(e, n);
  if (t)
    return Je(t, e);
}, Ee = async (n, e) => {
  const [t, r] = e.split(".");
  return !t || !r ? void 0 : (await n.app.model.domainModels.getDomainModel(t))?.entities.find((s) => s.name === r);
}, pe = (n, e) => {
  const t = e.generalization;
  return Ee(n, t.generalization);
}, qt = async (n, e) => {
  e.qualifiedName && await n.ui.editors.editDocument(
    { sdkType: "DomainModels$DomainModel", qualifiedName: e.qualifiedName.split(".")[0] },
    { id: e.documentId }
  );
}, We = (n) => ({
  check: async (e) => {
    const t = await Ze(n, e), r = $e(e), o = await et(n, e), s = tt(e), i = nt(e), a = ot(e), l = await rt(n, e), m = await st(n, e);
    return {
      errors: [
        ...t.errors,
        ...o.errors,
        ...s.errors,
        ...i.errors,
        ...a.errors,
        ...l.errors,
        ...m.errors,
        ...r.errors
      ],
      dependentElementIds: [
        ...t.dependentElementIds,
        ...o.dependentElementIds,
        ...s.dependentElementIds,
        ...a.dependentElementIds,
        ...l.dependentElementIds,
        ...m.dependentElementIds,
        ...r.dependentElementIds
      ]
    };
  },
  reservedErrorCodes: Xe
});
async function Ze(n, e) {
  const t = [], r = [];
  if (!e.model)
    return t.push({
      elementText: "Model resource",
      errorCode: B,
      errorDescription: "Model resource is required for an agent.",
      severity: "error"
    }), { errors: t, dependentElementIds: r };
  r.push(e.model.documentId);
  const o = await b(n, e.model);
  return o ? o.excluded && t.push({
    elementText: "Model resource",
    errorCode: _,
    errorDescription: `Model ${e.model.qualifiedName} is excluded from the project and cannot be used.`,
    severity: "error"
  }) : t.push({
    elementText: "Model resource",
    errorCode: x,
    errorDescription: `Model ${e.model.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
    severity: "error"
  }), { errors: t, dependentElementIds: r };
}
const $e = (n) => {
  const e = [];
  return n.model && !n.selectedModel && e.push({
    elementText: "Model version",
    errorCode: j,
    errorDescription: "Model version is required for an agent",
    severity: "error"
  }), { errors: e, dependentElementIds: [] };
};
async function et(n, e) {
  const t = [], r = [];
  if (!e.entity)
    return e.variables.length > 0 && t.push({
      elementText: "Context entity",
      errorCode: L,
      errorDescription: "Context entity is required for an agent when using variables.",
      severity: "error"
    }), { errors: t, dependentElementIds: [] };
  r.push(e.entity.documentId);
  const o = await Ee(n, e.entity.qualifiedName);
  if (!o)
    return t.push({
      elementText: "Context entity",
      errorCode: P,
      errorDescription: `Context entity ${e.entity.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
      severity: "error"
    }), { errors: t, dependentElementIds: r };
  const s = await me(n, o), i = e.variables.filter((a) => !s.includes(a.key)).map((a) => a.key);
  return i.length > 0 && t.push({
    elementText: "Context entity",
    errorCode: G,
    errorDescription: `Variable${i.length > 1 ? "s" : ""} ${i.join(", ")} could not be found in context entity ${e.entity.qualifiedName}.`,
    severity: "warning"
  }), { errors: t, dependentElementIds: r };
}
function tt(n) {
  const e = [];
  return n.usageType === Ye.Task && !n.userPrompt && e.push({
    elementText: "User prompt",
    errorCode: v,
    errorDescription: "User prompt is required for an agent.",
    severity: "error"
  }), { errors: e, dependentElementIds: [] };
}
function nt(n) {
  const e = [];
  return n.usageType || e.push({
    elementText: "Agent type",
    errorCode: Q,
    errorDescription: "Agent type is required for an agent.",
    severity: "error"
  }), { errors: e, dependentElementIds: [] };
}
function ot(n) {
  const e = [];
  if (n.toolChoice !== De.Tool)
    return { errors: e, dependentElementIds: [] };
  if (!n.toolChoiceToolName)
    e.push({
      elementText: "Tool choice",
      errorCode: Y,
      errorDescription: "A specific tool must be selected when tool choice is set to 'Tool'.",
      severity: "error"
    });
  else {
    const t = n.tools.find((r) => r.name === n.toolChoiceToolName);
    t ? t.enabled || e.push({
      elementText: "Tool choice",
      errorCode: H,
      errorDescription: `Tool '${n.toolChoiceToolName}' is not active. Please activate it, or select a different tool.`,
      severity: "error"
    }) : e.push({
      elementText: "Tool choice",
      errorCode: X,
      errorDescription: `Tool '${n.toolChoiceToolName}' is not in the agent's tools list. Please update your tool selection or change the tool choice.`,
      severity: "error"
    });
  }
  return { errors: e, dependentElementIds: [] };
}
async function rt(n, e) {
  const t = [], r = [];
  for (const o of e.tools) {
    if (!o.enabled)
      continue;
    r.push(o.document.documentId);
    const s = await Te(n, o.toolType, o.document), i = Ce(o.toolType);
    if (!s)
      t.push({
        elementText: "Tools",
        errorCode: U,
        errorDescription: `${i} ${o.document.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
        severity: "error"
      });
    else if (s.excluded)
      t.push({
        elementText: "Tools",
        errorCode: q,
        errorDescription: `${i} ${o.document.qualifiedName} is excluded from the project and cannot be used.`,
        severity: "error"
      });
    else if (Re(s)) {
      const { errors: a } = it(s, o);
      t.push(...a);
    }
  }
  return { errors: t, dependentElementIds: r };
}
async function st(n, e) {
  const t = [], r = [];
  if (e.knowledgebaseTools)
    for (const o of e.knowledgebaseTools) {
      if (!o.enabled)
        continue;
      r.push(o.document.documentId);
      const s = await b(n, o.document);
      s ? s.excluded && t.push({
        elementText: "Knowledge bases",
        errorCode: V,
        errorDescription: `Knowledge base ${o.document.qualifiedName} is excluded from the project and cannot be used.`,
        severity: "error"
      }) : t.push({
        elementText: "Knowledge bases",
        errorCode: K,
        errorDescription: `Knowledge base ${o.document.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
        severity: "error"
      });
    }
  return { errors: t, dependentElementIds: r };
}
function it(n, e) {
  const t = [];
  return n.microflowReturnType.$Type !== "DataTypes$StringType" && t.push({
    elementText: "Tools",
    errorCode: F,
    errorDescription: `Microflow ${e.document.qualifiedName} must return a String.`,
    severity: "error"
  }), Oe(n) && t.push({
    elementText: "Tools",
    errorCode: z,
    errorDescription: `Microflow ${e.document.qualifiedName} has invalid parameter types. Only primitive types and GenAICommons.Request or GenAICommons.Tool objects are supported.`,
    severity: "error"
  }), { errors: t, dependentElementIds: [] };
}
async function at(n) {
  await n.ui.tabs.open(
    {
      title: Ne,
      icon: g
    },
    {
      uiEntrypoint: Ie,
      componentName: Ae
    },
    {
      isSingleton: !0
    }
  );
}
const ct = (n) => ({
  check: async (e) => {
    const t = await dt(n, e), r = await lt(n, e);
    return {
      errors: [...t.errors, ...r.errors],
      dependentElementIds: [...t.dependentElementIds, ...r.dependentElementIds]
    };
  },
  reservedErrorCodes: Qe
});
async function dt(n, e) {
  const t = [], r = [];
  if (!e.endpoint)
    return t.push({
      errorCode: ee,
      errorDescription: "Endpoint is required for a consumed MCP service.",
      severity: "error",
      elementText: "Endpoint"
    }), { errors: t, dependentElementIds: r };
  r.push(e.endpoint.documentId);
  const o = await D(n, e.endpoint);
  return o ? o.excluded ? t.push({
    errorCode: oe,
    errorDescription: `The constant ${e.endpoint.qualifiedName} is excluded from the project and cannot be used.`,
    severity: "error",
    elementText: "Endpoint"
  }) : T(o) || t.push({
    errorCode: ne,
    errorDescription: `The constant ${e.endpoint.qualifiedName} must be of type string.`,
    severity: "error",
    elementText: "Endpoint"
  }) : t.push({
    errorCode: te,
    errorDescription: `The constant ${e.endpoint.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
    severity: "error",
    elementText: "Endpoint"
  }), { errors: t, dependentElementIds: r };
}
async function lt(n, e) {
  const t = [], r = [];
  if (!e.authenticationMicroflow)
    return { errors: t, dependentElementIds: r };
  r.push(e.authenticationMicroflow.documentId);
  const o = await he(
    n,
    e.authenticationMicroflow
  );
  return o ? o.excluded ? (t.push({
    errorCode: ae,
    errorDescription: `Microflow ${e.authenticationMicroflow.qualifiedName} is excluded from the project and cannot be used.`,
    severity: "error",
    elementText: "Authentication microflow"
  }), { errors: t, dependentElementIds: r }) : (o.objectCollection.objects?.filter((s) => s.$Type === "Microflows$MicroflowParameterObject").length !== 0 && t.push({
    errorCode: ie,
    errorDescription: `Microflow ${e.authenticationMicroflow.qualifiedName} cannot have input parameters.`,
    severity: "error",
    elementText: "Authentication microflow"
  }), (o.microflowReturnType.$Type !== "DataTypes$ListType" || o.microflowReturnType.entity !== we.HttpHeader) && t.push({
    errorCode: se,
    errorDescription: `Microflow ${e.authenticationMicroflow.qualifiedName} must return a list of System.HttpHeader objects.`,
    severity: "error",
    elementText: "Authentication microflow"
  }), { errors: t, dependentElementIds: r }) : (t.push({
    errorCode: re,
    errorDescription: `Microflow ${e.authenticationMicroflow.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
    severity: "error",
    elementText: "Authentication microflow"
  }), { errors: t, dependentElementIds: r });
}
const ut = (n) => ({
  check: async (e) => {
    const t = await mt(n, e);
    return {
      errors: [...t.errors],
      dependentElementIds: [...t.dependentElementIds]
    };
  },
  reservedErrorCodes: je
});
async function mt(n, e) {
  const t = [], r = [], o = e.providerFields;
  if (!o.key)
    return t.push({
      errorCode: ce,
      errorDescription: "Key is required for a Mendix Cloud knowledge base.",
      severity: "error",
      elementText: "Knowledge base key"
    }), { errors: t, dependentElementIds: r };
  r.push(o.key.documentId);
  const s = await D(n, o.key);
  return s ? s.excluded ? t.push({
    errorCode: ue,
    errorDescription: `The constant ${o.key.qualifiedName} is excluded from the project and cannot be used.`,
    severity: "error",
    elementText: "Knowledge base key"
  }) : T(s) || t.push({
    errorCode: le,
    errorDescription: `The constant ${o.key.qualifiedName} must be of type string.`,
    severity: "error",
    elementText: "Knowledge base key"
  }) : t.push({
    errorCode: de,
    errorDescription: `The constant ${o.key.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
    severity: "error",
    elementText: "Knowledge base key"
  }), { errors: t, dependentElementIds: r };
}
const Et = (n) => ({
  check: async (e) => {
    const t = await pt(n, e);
    return {
      errors: [...t.errors],
      dependentElementIds: [...t.dependentElementIds]
    };
  },
  reservedErrorCodes: He
});
async function pt(n, e) {
  const t = [], r = [], o = e.providerFields;
  if (!o.key)
    return t.push({
      errorCode: J,
      errorDescription: "Key is required for a Mendix Cloud model.",
      severity: "error",
      elementText: "Model key"
    }), { errors: t, dependentElementIds: r };
  r.push(o.key.documentId);
  const s = await D(n, o.key);
  return s ? s.excluded ? t.push({
    errorCode: $,
    errorDescription: `The constant ${o.key.qualifiedName} is excluded from the project and cannot be used.`,
    severity: "error",
    elementText: "Model key"
  }) : T(s) || t.push({
    errorCode: Z,
    errorDescription: `The constant ${o.key.qualifiedName} must be of type string.`,
    severity: "error",
    elementText: "Model key"
  }) : t.push({
    errorCode: W,
    errorDescription: `The constant ${o.key.qualifiedName} could not be found. Please make sure it exists and is correctly referenced.`,
    severity: "error",
    elementText: "Model key"
  }), { errors: t, dependentElementIds: r };
}
async function I(n) {
  await n.ui.tabs.open(
    { title: Pe, icon: g },
    {
      uiEntrypoint: Le,
      componentName: Ae
    },
    {
      isSingleton: !0
    }
  );
}
async function At(n, e) {
  try {
    n.app.projectChanges.addEventListener("elementsRenamed", async ({ elements: t }) => {
      await yt(n, t), await Dt(n, t), await Ct(n, t), await Tt(n, t);
    });
  } catch (t) {
    e.error("Failed to set up elementsRenamed event listener", { error: t });
  }
}
function ft(n, e, t, r, o) {
  n.qualifiedName = e, t.app.model.customBlobDocuments.updateDocumentContent(r, o);
}
function gt(n, e, t) {
  const r = e.find((s) => s.documentType === Be);
  if (r) {
    const [s, i] = n.qualifiedName.split(".");
    if (r.oldName.qualifiedName === s)
      return `${r.newName.qualifiedName}.${i}`;
  }
  return e.find((s) => s.oldName.qualifiedName !== n.qualifiedName ? !1 : t ? t.includes(s.documentType) : !0)?.newName.qualifiedName;
}
function d(n, e, t, r, o) {
  const s = gt(t, r, o);
  s && ft(t, s, n, e.$ID, e.contents);
}
async function yt(n, e) {
  const t = await n.app.model.customBlobDocuments.getDocumentsOfType(y);
  for (const r of t) {
    const o = await n.app.model.customBlobDocuments.getDocumentById(r.id);
    if ("document" in o && o.document) {
      if (o.document.contents.model && d(n, o.document, o.document.contents.model, e, [
        p
      ]), o.document.contents.entity && d(n, o.document, o.document.contents.entity, e, [
        xe
      ]), o.document.contents.tools)
        for (const s of o.document.contents.tools)
          d(n, o.document, s.document, e, [
            _e,
            A
          ]);
      if (o.document.contents.knowledgebaseTools)
        for (const s of o.document.contents.knowledgebaseTools)
          d(n, o.document, s.document, e, [
            f
          ]);
    }
  }
}
async function Dt(n, e) {
  const t = await n.app.model.customBlobDocuments.getDocumentsOfType(p);
  for (const r of t) {
    const o = await n.app.model.customBlobDocuments.getDocumentById(r.id);
    if ("document" in o && o.document && o.document.contents.provider === "MxCloudGenAI") {
      const s = o.document.contents.providerFields;
      if (!s.key)
        continue;
      d(n, o.document, s.key, e, [C]);
    }
  }
}
async function Tt(n, e) {
  const t = await n.app.model.customBlobDocuments.getDocumentsOfType(f);
  for (const r of t) {
    const o = await n.app.model.customBlobDocuments.getDocumentById(r.id);
    if ("document" in o && o.document && o.document.contents.provider === "MxCloudGenAI") {
      const s = o.document.contents.providerFields;
      if (!s.key)
        continue;
      d(n, o.document, s.key, e, [C]);
    }
  }
}
async function Ct(n, e) {
  const t = await n.app.model.customBlobDocuments.getDocumentsOfType(
    A
  );
  for (const r of t) {
    const o = await n.app.model.customBlobDocuments.getDocumentById(r.id);
    "document" in o && o.document && o.document.contents.endpoint && d(n, o.document, o.document.contents.endpoint, e, [
      C
    ]);
  }
}
const Ae = "extension/agent-editor";
class Rt {
  async loaded(e) {
    const t = ge(e);
    try {
      if (await t.app.model.customBlobDocuments.registerDocumentType({
        type: y,
        readableTypeName: "Agent",
        defaultContent: be(),
        consistencyCheckRegistration: We(t),
        serializationCallback: async (r) => JSON.stringify(r, null, 2)
      }), await t.app.model.customBlobDocuments.registerDocumentType({
        type: p,
        readableTypeName: "Model",
        defaultContent: Me(),
        consistencyCheckRegistration: Et(t),
        serializationCallback: async (r) => JSON.stringify(r, null, 2)
      }), await t.app.model.customBlobDocuments.registerDocumentType({
        type: A,
        readableTypeName: "Consumed MCP service",
        defaultContent: ke(),
        consistencyCheckRegistration: ct(t),
        serializationCallback: async (r) => JSON.stringify(r, null, 2)
      }), await t.app.model.customBlobDocuments.registerDocumentType({
        type: f,
        readableTypeName: "Knowledge base",
        defaultContent: Se(),
        consistencyCheckRegistration: ut(t),
        serializationCallback: async (r) => JSON.stringify(r, null, 2)
      }), e.runMode === "ui") {
        const { createStudioProLogger: r } = await import("./logger-gMbuSAUF.js"), { createAnalyticsService: o } = await import("./analytics-B0H4T-RD.js"), s = O(e), i = ve(e), a = r(s), l = await Ue(s), m = o(i, l, a);
        At(s, a), m.registerDocumentCreationListener(s);
        let R = !1;
        s.runtime.controller.addEventListener("connectionChanged", (c) => {
          R = c.isConnected, s.ui.messagePassing.sendMessage({
            type: E.ConnectionChanged,
            isConnected: c.isConnected
          });
        }), s.ui.messagePassing.addMessageHandler(
          async ({ messageId: c, message: fe }) => {
            fe.type === E.GetConnectionState && await s.ui.messagePassing.sendResponse(c, {
              isConnected: R
            });
          }
        ), s.runtime.controller.addEventListener("modelReloaded", async () => {
          try {
            await s.runtime.controller.executePreviewAction(
              Ve.PreviewAgentSync,
              {}
            ), await s.ui.messagePassing.sendMessage({ type: E.SyncSuccessful });
          } catch {
            await s.ui.messagePassing.sendMessage({ type: E.SyncFailed });
          }
        }), await s.ui.editors.registerEditorForCustomDocument({
          documentType: y,
          editorKind: "tab",
          editorEntryPoint: "agentEdit",
          iconLight: g,
          iconDark: g
        }), await s.ui.editors.registerEditorForCustomDocument({
          documentType: p,
          editorKind: "tab",
          editorEntryPoint: "modelEdit",
          iconLight: w,
          iconDark: w
        }), await s.ui.editors.registerEditorForCustomDocument({
          documentType: A,
          editorKind: "tab",
          editorEntryPoint: "consumedMCPServiceEdit",
          iconLight: N,
          iconDark: N
        }), await s.ui.editors.registerEditorForCustomDocument({
          documentType: f,
          editorKind: "tab",
          editorEntryPoint: "knowledgebaseEdit",
          iconLight: h,
          iconDark: h
        }), await s.ui.extensionsMenu.add({
          caption: u.AgentEditor.caption,
          menuId: u.AgentEditor.id,
          subMenus: [
            {
              caption: u.Onboarding.caption,
              menuId: u.Onboarding.id,
              action: () => I(s)
            },
            {
              caption: u.Compatibility.caption,
              menuId: u.Compatibility.id,
              action: () => at(s)
            }
          ]
        });
        try {
          const c = await k(s);
          Fe(c, l) && (await I(s), await Ke(s, l));
        } catch (c) {
          a.error("Failed to initialize onboarding:", c);
        }
      }
    } catch (r) {
      if (e.runMode === "ui") {
        const o = O(e), { createStudioProLogger: s } = await import("./logger-gMbuSAUF.js");
        s(o).error("Error during extension initialization:", r), await o.ui.messageBoxes.show(
          "error",
          `Error during extension initialization: ${r.message}`
        );
      }
      throw r;
    }
  }
}
const Ft = new Rt();
export {
  vt as D,
  Pt as M,
  _t as O,
  Ve as P,
  E as R,
  Ye as U,
  Ut as V,
  me as a,
  g as b,
  Lt as c,
  Gt as d,
  Ae as e,
  Ft as f,
  Ee as g,
  qt as h,
  Je as i,
  at as o
};
