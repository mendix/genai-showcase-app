...
function lo(e, u) {
  ...
  let { collection: P } = u, { disabledKeys: K } = u.selectionManager, T = a.useMemo(() => l || new Ge({
    ...
    let m = n?.current && n.current === v.relatedTarget, S = De(o.current, v.relatedTarget);
  ...
  var Y;
  let X = (Y = V?.parentKey) !== null && Y !== void 0 ? Y : null;
  var pe;
  ...
  a.useEffect(() => {
    if (ge() && V != null && U != null && U !== ue.current) {
      let m = u.selectionManager.isSelected(U), S = X != null ? u.collection.getItem(X) : null, J = S?.["aria-label"] || (typeof S?.rendered == "string" ? S.rendered : "") || "";
      var v;
  ...
  return a.useEffect(() => {
    ...
    onInteractOutside: (v) => {
      let m = Ie(v);
      De(n?.current, m) || De(i.current, m) || u.close();
    },
    isDisabled: !u.isOpen
  }), {
    labelProps: w,
...
function Ou(e, u) {
  var n;
  let o = e?.renderDropIndicator, i = e == null || (n = e.isVirtualDragging) === null || n === void 0 ? void 0 : n.call(e), t = a.useCallback((l) => {
    if (i || u?.isDropTarget(l)) return o ? o(l) : /* @__PURE__ */ g.createElement($o, {
      target: l
    });
  }, [
    u?.target,
...
function Ru({ state: e, props: u, listBoxRef: n }) {
  [u, n] = Be(u, n, Tn);
  let { dragAndDropHooks: o, layout: i = "stack", orientation: t = "vertical", filter: l } = u, r = ao(e, l), { collection: d, selectionManager: b } = r, $ = !!o?.useDraggableCollectionState, c = !!o?.useDroppableCollectionState, { direction: p } = Fn(), { disabledBehavior: C, disabledKeys: h } = b, P = Oe({
    usage: "search",
    sensitivity: "base"
