import { L as s, H as t, ae as j, af as y, r as c, R as v } from "./Icon-UiLWY9J1.js";
import { r as E } from "./index-CfghFsoU.js";
import { G as w, H as C } from "./index-CcjggGcx.js";
...
function B(e) {
  return {
    async loaded(n, i) {
      const o = E(n), { documentId: r } = i, l = document.getElementById("root");
...
const k = ({
  ...
  m.current = r;
  const f = c.useRef(o);
  f.current = o, c.useEffect(() => {
    const p = ({ documents: x }) => {
      const d = [];
      for (const a of x)
        m.current && a.documentType === w && a.moduleName && m.current.includes(a.moduleName) ? d.push("domainModel") : f.current && a.documentType === C ? d.push("projectSettings") : u.current.includes(a.documentId) && d.push(a.documentId);
      d.length > 0 && l.current(d);
    };
    return e.app.projectChanges.addEventListener("documentsChanged", p), () => {
      e.app.projectChanges.removeEventListener("documentsChanged", p);
    };
